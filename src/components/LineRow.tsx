                ...
                {seg.chords.map(chord => {
                  ...
                  return (
                    <div
                      ...
                      className={`chord-label relative z-10 ${isPlaying ? 'bg-[var(--accent)] text-white scale-110 shadow-md z-20 transition-all' : ''} ${isPreviewMode ? 'cursor-pointer hover:bg-[var(--accent-dim)]' : ''}`}
                    >
                      {chord.chord}

                      {/* Popup for editing an existing chord */}
                      {!isPreviewMode && editingChord?.chordId === chord.id && (
                ...
                {seg.chars.map(c => {
                  ...
                  return (
                    <span
                      ...
                      onClick={() => {
                        if (!isPreviewMode) onClickChar(c.position);
                      }}
                      onDragOver={e => { 
                        if (isPreviewMode) return;
                        e.preventDefault(); 
                        onDragOverChar(line.id, c.position); 
                      }}
                      onDragLeave={() => {
                        if (!isPreviewMode) onDragLeave();
                      }}
                      onDrop={e => {
                        if (!isPreviewMode) onDrop(e, line.id, c.position);
                      }}
                    >
                      {/* Invisible extension for drag over chord row */}
                      {!isPreviewMode && <span className="absolute -top-[1.4em] left-0 right-0 h-[1.4em] z-0" />}
                      
                      <span className="relative z-10">{c.char === ' ' ? '\u00A0' : c.char}</span>

                      {!isPreviewMode && isDropTarget && dragItem && (
                      ...
                      {!isPreviewMode && hasNewChordPopup && (
